...
const Profile = ({ navigation, route }) => {
  ...
  const verifyPasswordAndShowPassPhrase = async () => {
    try {
      const encryptedPassword = await SecureStorage.getItem('newPassword');
      const storedPassword = CryptoJS.AES.decrypt(
        encryptedPassword,
      ...
      if (storedPassword === password) {
        const encryptedSeedPhrase = await SecureStorage.getItem('encryptedSeedPhrase');
        const decryptedSeedPhrase = CryptoJS.AES.decrypt(
          encryptedSeedPhrase,
          config.privateKeyEncryptionString
        ).toString(CryptoJS.enc.Utf8);

        setPassPhrase(decryptedSeedPhrase);
        setPassword('');
        Animated.timing(fadeAnim, {
          toValue: 1,
          duration: 500,
    ...
    <>
      <View style={styles.container}>
        ...
        >
          ...
          <Text style={styles.sectionTitle}>Reveal Your Private Key</Text>
        </TouchableOpacity>
        ...
        >
          <FontAwesome name="key" size={20} color="#FEBF32" />
          <Text style={styles.sectionTitle}>Reveal Your Pass Phrase</Text>
        </TouchableOpacity>
            ...
            <View style={styles.modalContent}>
              ...
              {!privateKey && (
                <>
                  ...
                  <TouchableOpacity
                    style={[styles.verifyButton, privateKey && styles.disabledButton]}
        ...
        >
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              <Text style={styles.modalTitle}>
                {passPhrase ? 'Pass Phrase' : 'Enter Password'}
              </Text>
              {!passPhrase && (
                <>
                  <View style={styles.passwordContainer}>
                  ...
                  {error ? <Text style={styles.errorText}>{error}</Text> : null}
                  <TouchableOpacity
                    style={[styles.verifyButton, passPhrase && styles.disabledButton]}
                    onPress={verifyPasswordAndShowPassPhrase}
                    disabled={!!passPhrase}
                  >
                    <Text style={styles.verifyButtonText}>Verify</Text>
                  </TouchableOpacity>
              ...
              {passPhrase ? (
                <Animated.View
                  style={[styles.privateKeyContainer, { opacity: fadeAnim }]}
                >
                  <Text style={styles.privateKey}>{passPhrase}</Text>
                  <TouchableOpacity
                    style={styles.copyButton}
                    onPress={() => copyToClipboard(passPhrase)}
                  >
                    <FontAwesome name="copy" size={16} color="#FFF" />
                  </TouchableOpacity>
                </Animated.View>
